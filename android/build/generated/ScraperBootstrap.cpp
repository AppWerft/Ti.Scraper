...
#define TAG "de.appwerft.scraper"

using namespace v8;

static Persistent<Object> bindingCache;

static void Scraper_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Scraper.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::ScraperBindings::lookupGeneratedInit(
...
static void Scraper_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Scraper_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Scraper_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::ScraperBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry ScraperBinding = {
	"de.appwerft.scraper",
	Scraper_init,
	Scraper_dispose
};
...
Java_de_appwerft_scraper_ScraperBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.scraper", &ScraperBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::ScraperBindings::lookupGeneratedInit));
}
